import React from 'react'
import { useContext } from 'react'

const DriverPick = (props) => {
  
  return (
    <div className='confirm-panel'> 
    <div className='parent flex  flex-col'>
    <h1 onClick={()=>{
              props.setdriverPick(false)
              props.setVehiclePanel(true)
              props.findTripBtn(false)
         }}
          className='text-center'><i className=" font-semibold text-xl ri-arrow-down-wide-line"></i></h1>
        <h1 className="font-bold  text-xl p-2">Driver is on the way</h1>
        
            <div className='flex justify-between items-center'>
          <img className='w-32' src='data:image/jpeg;base64,'
         alt="UberGo"/>
           <div className='mr-5 mb-2'>
              <p className='font-semibold capitalize'>{props.ride?.captain.fullname.firstname + " " + props.ride?.captain.fullname.lastname}</p>
              <p className='font-bold text-xl'>{props.ride?.captain.vehicle.carPlate}</p>
              <p className='text-gray-600 capitalize text-md'>{props.vehicleType}</p>
              <p className='font-bold capitalize text-md'>OTP={props.ride?.otp}</p>
            
           </div>
            </div>

        
        <div className='ml-10 mb-10 mt-2 space-y-2'>
              <div>
                <div className='flex gap-4 text-xl font-bold '><h1 ><i className="ri-map-pin-2-fill"></i></h1>
                562/11A
                </div>
                <p className='text-lg text-gray-500'>{props.ride?.pickup}</p>
                </div>
              <div>
                <div className='flex gap-4 text-xl font-bold '><h1 ><i className="ri-map-pin-2-fill"></i></h1>
                562/11A
                </div>
                <p className='text-lg text-gray-500'>{props.ride?.destination}</p>
                </div>
               

              <div className=' flex gap-2'>
              <div className='text-lg font-bold '>
               <p> <i className=" text-2xl ri-cash-line"></i></p>
               </div>
               <p className='font-bold text-xl'>{props.ride?.fare}</p>
              </div>
              <p className='text-lg text-gray-500'>Cash</p>
              
        </div>

        <div>
           
        </div>
    </div>
</div>
  )
}

export default DriverPick
